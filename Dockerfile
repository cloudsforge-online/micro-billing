...
# ----------------------------------------------------------------------------------- build
# `tsc --noEmit` rather than an emit: tsx runs the TypeScript sources directly. What this stage
# buys is that a type error fails the image build instead of the first request.
FROM deps AS build
...
# init container or a Kubernetes Job before this ever starts.
