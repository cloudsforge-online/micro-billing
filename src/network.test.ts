 ...
 *
 * billing serves BOTH estates from one process since the network consolidation (micro-deploy
...
describe('the handle a request gets', () => {
  ...
  it('REFUSES when this deployment holds no handle for that network', () => {
    ...
    // write a testnet purchase into the mainnet database AND post it to the mainnet ledger,
    // leaving two sides that agree with each other and are both wrong.
...
describe('the operational endpoints are exempt, and only they', () => {
  ...
  it('does not exempt anything that reads or writes', () => {
    for (const p of ['/products', '/v1/purchases', '/v1/entitlements']) {
