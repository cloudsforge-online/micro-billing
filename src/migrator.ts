...
try {
  const result = await migrate(sql as unknown as Sql, MIGRATIONS, {
    service: SERVICE,
    // Zero for a new service, which makes this a no-op. The estate's entitlements table is not
    // baselined into this one: it has no scope, no expiry and no revocation, so no version here
    // describes it. Its rows arrive through a migration with source = 'migration' instead.
