...
export interface RecycleRow {
  ...
  readonly grossShards: bigint
  readonly refundedShards: bigint
  readonly recycleBps: number
  /** The database's arithmetic, read back — never recomputed here. See migration 10. */
...
interface Row {
  ...
  readonly gross_shards: string
  readonly refunded_shards: string
...
const COLUMNS = `id, asset_code, period_start, period_end, gross_shards::text,
                 refunded_shards::text, recycle_bps, amount_shards::text, status, journal_entry_id`
 ...
 */
function shards(value: string | null | undefined, field: string): bigint {
...
function toRow(row: Row): RecycleRow {
  return {
    ...
    grossShards: shards(row.gross_shards, 'gross_shards'),
    refundedShards: shards(row.refunded_shards, 'refunded_shards'),
    recycleBps: row.recycle_bps,
    amountShards: shards(row.amount_shards, 'amount_shards'),
...
export async function periodBasis(
  ...
  return {
    grossShards: shards(row.gross, 'gross_shards'),
    refundedShards: shards(row.refunded, 'refunded_shards'),
  }
}
...
export async function claimPeriod(
  ...
  await sql`
    ...
    values (
      ...
      ${input.basis.grossShards.toString()}::numeric,
      ${input.basis.refundedShards.toString()}::numeric,
  ...
  try {
    const entry = await deps.ledger.postEntry({
      ...
      metadata: {
        ...
        grossShards: row.grossShards.toString(),
        refundedShards: row.refundedShards.toString(),
      },
      postings: feeRecyclePostings({ assetCode: deps.assetCode, amount: row.amountShards }),
    })
    entryId = entry.id
  } catch (err) {
    if (err instanceof InsufficientFundsError) {
      ...
      deps.logger.warn('fee recycle deferred — platform fee revenue would go negative', {
        recycleId: row.id,
        periodStart,
        amountShards: row.amountShards.toString(),
      })
      return 'deferred'
    }
