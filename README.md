...
> at [`src/billing/`](https://github.com/cloudsforge-online/micro-agora/tree/main/src/billing).
...
`src/entitlements.ts` names them, and each is a live defect the schema now refuses:
...
4. **A service-readable API.** The estate's `GET /entitlements` is Bearer-only
   (`monetization.ts`), so **no service can ask whether a user owns anything** — a world server
...
(`src/entitlements.ts`, applied). The schema still carries
`granted_at timestamptz not null default now()` (`src/migrations.ts`) — the default remains for
a hand-written row, but every grant this service makes overrides it.

Why (`src/entitlements.ts`): every timestamp the activity rule reads — `granted_at`,
...
(`src/server.ts`); scope is checked per-route and, except on `/internal/…`, **only for service
principals**.
...
| `GET` | `/products` | **no auth** | — | the catalogue. Public: a catalogue behind a token cannot be browsed (`src/server.ts`) |
| `POST` | `/purchases` | user or admin; service needs `billing:grant` | **required** | buys a SKU: one ledger entry, one entitlement, one transaction. 201 fresh, **200 on a replay** (`src/server.ts`, key) |
| `GET` | `/entitlements` | user or admin; service needs `billing:read` | — | what the calling user owns. Supports `at`, `scope`, `sku`, `includeInactive`, `limit` (`src/server.ts`) |
| `GET` | `/internal/entitlements/:userId` | **service only**, `billing:read` | — | **the service-readable API.** A user token is refused here **even for its own id** (`src/server.ts`, refusal) |
| `POST` | `/entitlements/:id/revoke` | user or admin; service needs `billing:grant` | — | revokes, optionally refunding. **A reason is required either way** (`src/server.ts`, reasoning) |
| `GET` | `/subscriptions` | user or admin; service needs `billing:read` | — | the caller's subscriptions (`src/server.ts`) |
...
(`src/server.ts`). That is not an oversight: the route exists for services, users have
`GET /entitlements` which runs the same query, and "a route that quietly accepted both would make
the scoped-token boundary decorative" (`src/server.ts`).
...
both, the request is a 400** (`src/server.ts`). The reason is stated at `src/server.ts`:
...
rather than answered with the first request's result (`src/server.ts`).

`scope` on a read is **parsed, not passed through**: it is a lookup key, and an unvalidated one is a
query parameter that decides which rows a service is shown (`src/server.ts`).

---

## One purchase, one entry — and there are two idempotency keys

`src/purchases.ts` is the argument, and it is worth reading before changing anything here.
...
**Why the HTTP call is inside the transaction** (`src/purchases.ts`): it holds a database
...
(`src/entitlements.ts`). That topic is the one `micro-worlds` subscribes to.

---

## Background work

Leased jobs only. **The lease key names the contended resource** (`src/jobs.ts`).

| Job | Lease key | Cadence | What two replicas do |
| --- | --- | --- | --- |
| `outbox.relay` | `stream` | 1s | one claims the stream (`src/jobs.ts`) |
| `billing.entitlement.expire` | `global` | 60s | nothing breaks, **but two sweeps would emit two `revoked` events for one expiry — and a subscriber that tears down a world on that event would do it twice.** The UPDATE uses `SKIP LOCKED` as a second line of defence (`src/jobs.ts`, reasoning) |
...
| `billing.renewals.scan` | `global` | 5min | enqueues renewal jobs; two scans enqueue each twice, collapsed by the `(kind, key)` uniqueness, but the second scan is pure waste against the same index (`src/jobs.ts`) |
| `billing.idempotency.reap` | `global` | 24h | nothing, but two long DELETEs compete for the row locks at the head of every purchase (`src/jobs.ts`) |
...
never how long access lingers (`src/jobs.ts`).
...
which must be scoped to its title — exist from the start** (`src/migrations.ts`).

| Constraint | Refuses | Why it is here rather than in a handler |
| --- | --- | --- |
| `entitlements.scope` CHECK — `'platform'`, `like 'title:%'`, `like 'community:%'` | an unscoped or arbitrarily-scoped grant | **without a scope no service can ask "does this user own X for this title", which is why a purchased private world is never built.** Making it a column with a domain rather than a convention is what stops the next writer omitting it (`src/migrations.ts`) |
| `entitlements_expiry_after_grant` — `expires_at is null or expires_at > granted_at` | an entitlement that expires before it starts | it also pins the two timestamps into one comparable domain at the storage layer, which is the same invariant the clock-domain note above protects in the application (`src/migrations.ts`) |
| `entitlements.source` CHECK — `purchase`, `subscription`, `grant`, `reward`, `migration` | a provenance nobody enumerated | "where did this entitlement come from" is the first question in a dispute, and an unenumerated answer is not one (`src/migrations.ts`) |
| `entitlements.quantity numeric(78,0) check (quantity >= 0)` | a negative holding | `numeric(78,0)` for the same reason every quantity here is: a JSON number cannot carry it (`src/migrations.ts`) |
| `entitlements_live_idx`, a **partial** index `where revoked_at is null` | — | the service-readable lookup's access path. **Partial on the live set, because a revoked grant is history and the question is always about now** (`src/migrations.ts`) |
| `entitlements_expiring_idx`, partial `where revoked_at is null and expires_at is not null` | — | the expiry sweep's access path; perpetual grants are not scanned (`src/migrations.ts`) |
| `payouts_net_consistent` — `platform_fee <= gross and net = gross - platform_fee` | a payout whose parts do not add up | **this is the arithmetic reconciliation cannot see**: such a payout balances perfectly as a journal entry while paying the wrong amount. It is `isPayoutConsistent` from `contracts-money`, "enforced where a psql session has to obey it too" (`src/migrations.ts`, reasoning) |
| `payouts.journal_entry_id` | — | a payout is a ledger movement plus optionally a withdrawal, **never a separate money system**, and this column is what makes that checkable rather than aspirational (`src/migrations.ts`) |
| `entitlements.journal_entry_id` | — | the entry that paid for the grant. A refund reverses **this** id and revokes the row — one operation with one identifier, rather than a hunt through the journal (`src/migrations.ts`) |
...
their holder identically (`src/migrations.ts`).
...
| `BILLING_DATABASE_URL` | — | **required** (`src/env.ts`). Rule 1 — and note what is deliberately absent: no URL for the ledger's database (`src/env.ts`) |
| `BILLING_DATABASE_POOL_MAX` | `10` | 1–100. Larger than the database's budget divided by the replica count exhausts Postgres for everything else the moment billing scales (`src/env.ts`) |
...
| `BILLING_LEDGER_URL` | — | **required, and validated as an absolute `http(s)` URL** at boot rather than at first purchase (`src/env.ts`) |
...
| `BILLING_LEDGER_DEADLINE_MS` | `5000` | 250–30000. **It bounds how long the purchase transaction stays open**, because the posting happens inside it. Too generous and a slow ledger holds a database connection per in-flight purchase; too tight and a healthy retry never completes (`src/env.ts`, reasoning) |
...
| --- | --- | --- |
| `micro-ledger` | `POST /entries` (`src/ledger.ts`) and `POST /entries/:id/reverse` for a refund (`src/ledger.ts`), verified against `ledger/src/server.ts` and, reached with a token exchanged from `BILLING_IDENTITY_CREDENTIAL` | **fail closed, and structurally so.** The posting happens *inside* the purchase transaction (`src/purchases.ts`), so a ledger outage fails the purchase whole: no entitlement is granted, no `granted` event is emitted, and the customer is told. The alternative grants the thing before the money moves |
...
(`src/entitlements.ts`) — that topic is what finally provisions the private world that today is
...
* **Payouts are modelled and not driven.** `payouts` carries the consistency constraint and the
  `journal_entry_id` seam (`src/migrations.ts`), and no job moves a row through
